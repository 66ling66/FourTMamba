...
sys.path.append('/media/jaw/data/psw/FourierTMamba')
...
def create_train_val_dataloader(opt, logger):  # train loader 和 val loader 一起构建
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        if phase == 'train':
            ...
            # todo 这里构建datasets 我们要构建自己的datasets
...
def main():
    ...
    if resume_state:  # resume training
        ...
        model= create_model(opt)
        ckpt_dict = torch.load(opt['pretrained_ckpt'])
        model.net_g.body[0].estimator.DecomNet.load_state_dict(ckpt_dict,strict=False)
        # model.net_g.body[0].estimator.decomnet.load_state_dict(ckpt_dict,strict=False)
