    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            ...
            for pred in preds:
                ...
                # todo 计算傅里叶损失
                out_gt = torch.fft.rfft2(self.gt)
                pha_gt = torch.angle(out_gt)
                amp_gt = torch.abs(out_gt)

                out_pred = torch.fft.rfft2(pred)
                pha_pred = torch.angle(out_pred)
                amp_pred = torch.abs(out_pred)

                l_pix += 0.05 * self.cri_pix(pha_pred, pha_gt)
                l_pix += 0.05 * self.cri_pix(amp_pred, amp_gt)
